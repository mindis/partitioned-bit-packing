...
bool abstract_vector_test(const char* test_name, AbstractBitPackedVector& v, size_t distinct_values, bool expect_ok = true) {
    ...
    for (size_t n = 0; n < num_elements; ++n) {
        if (data[n] != v.get(n)) {
            if (expect_ok) printf("Error: Pos %u, Expected %u, Got %u\n", n, data[n], v.get(n));
            ok = false;
            break;
        }
    }
...
bool run_vector_tests() {
    bool ok = true;
    ok &= abstract_vector_test("BPV (1)", BasicBitPackedVector(4), 10);
    ok &= abstract_vector_test("BPV (2)", BasicBitPackedVector(10), 1000);
